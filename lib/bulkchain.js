...
var txidToRawTransaction = function ( txid, callback) {
    ...
    client.getRawTransaction( txid, verbose, function (err, rawtransaction) {
        logger.silly({'functionname': txidToRawTransaction, 'rawtransaction': rawtransaction})
...
var slow_rawTransactionToInputDetail = function ( rawtransaction, callback ) {
    ...
    var q_txidToInputDetail = async.queue(function (task, callback) {
        ...
        txidToInputDetail(task.txid, task.vout, function (err, inputDetail) {
            if (inputDetail) {
                logger.silly({'q_txidToInputDetail': q_txidToInputDetail.length(), 'txid': task.txid})
                inputDetailArr.push({'txid': task.txid, 'value': inputDetail[0].value, 'n': inputDetail[0].n, 'destroy_start':inputDetail[0].destroy_start})
                callback();
            }
            else {
                logger.silly('@@@RELOAD@@@');
...
var rawTransactionToTransactionSignature = function rawTransactionToTransactionSignature(rawtransaction, callback) {
    ...
    function shipTransactionSignature (err, transactionsignature){
        logger.debug({functionname: 'transactionsignature', transactionsignature: transactionsignature})
...
//        
        ...
        transactionsignature.satoshi_seconds_destroyed = parseInt((ss.max(transactionsignature.satoshi_seconds_destroyed_sum)) + .5)
        transactionsignature.satoshi_seconds_destroyed = parseInt((ss.sum(transactionsignature.satoshi_seconds_destroyed_concat)) + .5)
...
var dateRangeToTransactionSignature = function dateRangeToTransactionSignature(startdate, enddate, callback) {
    ...
    var q_rawTransactionToTransactionSignature = async.queue(function (task, callback) {
        logger.silly({functionname:'q_rawTransactionToTransactionSignature', blocks_in_queue: q_blockHashToTxid.length(), txid_in_queue: q_txidToRawTransaction.length(), rawtransaction_in_queue: q_rawTransactionToTransactionSignature.length()})
