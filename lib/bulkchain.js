...
var slow_rawTransactionToInputItem = function ( rawtransaction ) {
    var promises = rawtransaction.vin.map(function(vin) {
        txidToInputItem(vin.txid, vin.vout)
        .then(function (inputItem) {
            return (inputItem);
        });
    });
    return Promise.all(promises).then(function(inputItem) {
        return inputItem;
    });
};
...
var rawTransactionToTransactionSignature = function (rawtransaction) {
    return rawTransactionToInputItem(rawtransaction)
    .then(function(inputItem) {
        rawtransaction.vin_detail = inputItem;
