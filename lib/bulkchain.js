...
var bisect = function* (params) {
    //console.log({bisect: params});
    yield (Promise.props ({
            targetTime: params.targetTime,
            latestBlockCount: params.latestBlockCount,
            latestBlockTime: params.latestBlockTime,
            earliestBlockCount: params.earliestBlockCount,
            earliestBlockTime: params.earliestBlockTime,
            blockCountGuess: params.blockCountGuess,
            blockTimeGuess: params.blockTimeGuess,
            blockCountHigh: params.blockCountHigh,
            blockCountLow:  params.blockCountLow,
            blockTimeHigh: params.blockTimeHigh || params.latestBlockTime,
            blockTimeLow: params.blockTimeLow || params.earliestBlockTime
        })
        .then(function(guess) {
            //console.log({bisect: guess});
            if(guess.targetTime < guess.blockTimeLow) {
                console.log('way too low *******************');
                guess.blockCountWinner = params.earliestBlockCount;
                return guess;
            }
            if(guess.targetTime > guess.blockTimeHigh) {
                console.log('way too high *******************');
                guess.blockCountGuess = guess.blockTimeHigh;
                guess.blockCountWinner = params.latestBlockCount;
                return guess;
            }
            if(guess.blockCountLow <= guess.targetTime <= guess.latestBlockTime) {
                console.log('in range ********************');
                if(guess.blockTimeGuess < guess.targetTime) {
                    console.log('in range low ********************');
                    guess.blockCountHigh = guess.blockCountGuess;
                    guess.blockTimeHigh = guess.blockTimeGuess;
                } else {
                    console.log('in range high ********************');
...
var slow_timeToBlockCount = function* (targetTime) {
    ...
    .then(function(theend) {
        console.log(theend);
        return theend.blockCountWinner;
    })
    .catch((err) => {
        logger.error(err);
    });
