...
suite('bulkchain:', function(done) {
    ...
    test('rawTransactionToTransactionSignature (magic)', function (done) {
        ...
        function cb_rawTransactionToTransactionSignature(transactionsignature) {
            ...
            assert(transactionsignature.tx_time)
            assert(transactionsignature.tz_offset)
            assert(transactionsignature.tx_year)
            assert(transactionsignature.tx_quarter)
            assert(transactionsignature.tx_month)
            assert(transactionsignature.tx_day_of_month)
            assert(transactionsignature.tx_day_of_year)
            assert(transactionsignature.tx_day_of_week)
            assert(transactionsignature.tx_week_of_year)
            assert(transactionsignature.tx_year_of_week)
            assert(transactionsignature.tx_hour)
            assert(transactionsignature.tx_minute)
            assert(transactionsignature.tx_second)
            assert.equal(transactionsignature.satoshi_seconds_destroyed_sum,37908750491)
            done()
        }
    })
    ...
    test('dateRangeToTransactionSignature (time trial 1 day)', function (done) {
        ...
        var starttime = 1438412400 // date --date "2015-08-01 00:00:00" +%s
        var endtime =   1438413000 // date --date "2015-08-01 00:10:00" +%s
        // OSX 
        //         var starttime = 1436943600 // date -j -f %Y%m%d%H%M%S 20150715000000 +%s
        //         var endtime =   1437030000 // date -j -f %Y%m%d%H%M%S 20150716000000 +%s
