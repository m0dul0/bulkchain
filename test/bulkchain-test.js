...
suite('bulkchain:', function(done) {
    ...
    test('txidToInputDetail (magic)', function () {
        let txid = '25d4deffa4ac3b239565804decdde7c91eae489330a746ea486a3e0bdb3214b0';
        let n = 91;
        var inputDetail_n = bulkchain.txidToInputDetail(txid, n)
            .then(function(inputdetail) {
                return(inputdetail.n);
            });
        return expect(inputDetail_n).to.eventually.equal(n);
    });
    test('rawTransactionToInputDetail (magic)', function () {
        ...
        var inputDetailArrLength = bulkchain.rawTransactionToInputDetail(rawtransaction)
            .then(function(inputDetailArr) {
                return(inputDetailArr.length)
            });
        return expect(inputDetailArrLength).to.eventually.equal(3);
    });
    test('rawTransactionToInputDetail (block reward aka no inputs)', function () {
        ...
        var inputDetailArrLength = bulkchain.rawTransactionToInputDetail(rawtransaction)
            .then(function(inputDetailArr) {
                return(inputDetailArr[0])
            });
        return expect(inputDetailArrLength).to.eventually.equal(undefined);
    })
    ...
    test('rawTransactionToTransactionSignature (magic)', function () {
        ...
        return expect(transactionSignatureArr).to.eventually.equal(1438888798);
    });
    ...
    test('dateRangeToTransactionSignature (time trial 1 block)', function () {
       ...
       let starttime = 1441453061
       let endtime = 1441453062
