...
suite('bulkchain:', function(done) {
    ...
    test('rawTransactionToInput (magic)', function (done) {
        ...
        function cb_rawTransactionToInputDetail(err, inputdetail) {
            assert(inputdetail.length = JSON.parse(rawtransaction).vin.length)
            done()
        }
    })
})
